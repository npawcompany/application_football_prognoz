...
Reading order: disclaimer → teams → **исход 1/X/2** → **табло счёта** → контекст → AI. Numbers before prose.

## Probability
Card **Исход матча**. Three equal tiles (1 / X / 2) with Fira Code percents. Favorite tile gets a 1px status border and the caption **Фаворит**. Under the tiles: a thin segmented mix bar (not a fused 44px bar — labels live on the tiles so color is not the only encoding).
...
- Left: large Fira Code `H:A`, then a green chip `{n}% · самый вероятный`.
